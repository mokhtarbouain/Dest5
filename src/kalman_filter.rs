...
impl KalmanFilter2D {
    ...
    fn update(&mut self, measurement: Vector2) {
        ...
        let innovation_covariance_det = innovation_covariance.determinant();
        if innovation_covariance_det > 0.0 {
            let innovation_covariance_inv = innovation_covariance.inverse();
            let kalman_gain = self.error_covariance * innovation_covariance_inv;
